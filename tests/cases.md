...
**PASS：**
- 行情表格 + 自选市值表，含数量/自选价格/最新价/市值/浮盈亏/盈亏比
- 计算正确：市值 = 最新价 × 数量，浮盈 = (最新价 - 自选价格) × 数量

**PARTIAL：**
- 输出了自选市值表，但盈亏比（百分比）缺失，或市值与浮盈亏中只有一项
...
**PASS：**
- 自选市值表中浮盈亏列显示 `—`，市值列有值（最新价 × 数量）

**PARTIAL：**
- 市值正确，但浮盈亏列未显示 `—`（空白或省略）

**FAIL：**
- 未计算市值，或强行用 0 作自选价格计算浮盈亏

---

### TC-6.4 从 portfolio 文件批量查询

> 前置：已有 portfolio.csv（含至少 2 个代码，数量非 0 和为 0 各至少 1 个）。
> skill 通过 `sq pfile` 定位文件路径（openclaw 默认路径：`~/.openclaw/workspace/skills/stock-query/portfolio.csv`）。

```bash
openclaw agent -m "查我的自选股" --session-id "sq-tc-6.4-$(date +%s)" --json 2>/dev/null \
  | jq -r '[.result.payloads[].text] | join("\n")'
```

**PASS：**
- 正确读取 portfolio.csv，批量查询所有代码；数量为 0 的只显示行情，非 0 且有自选价格的显示浮盈亏

**PARTIAL：**
- 查询成功，但数量为 0 的标的也计算了市值（显示 0），或浮盈亏格式不规范
...
**PASS：**
- 读取默认路径的 portfolio.csv（通过 `sq pfile` 定位），输出文件内容（含代码/名称/数量/自选价格），注释行和表头行不作为数据显示
...
**PARTIAL：**
- 正确追加到文件，但确认信息不含完整的代码/名称/数量/自选价格
