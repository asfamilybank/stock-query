...
```
> **安全说明**：该接口为 HTTP（无 TLS），存在被篡改或窃听的理论风险；请求中仅包含基金代码（如 `{code}.js`），不包含用户账号、密码等敏感信息。
...
- **场外基金估值接口（fundgz.1234567.com.cn）**：当前仅提供 HTTP（无 TLS）。请求中仅发送基金代码，不涉及用户身份或敏感数据；若对传输完整性/隐私有更高要求，可优先依赖备用接口（东方财富 HTTPS）或仅使用确认净值。
- **portfolio_file 配置**：`skill.yaml` 中为可选配置项，表示持仓文件路径。**本 skill 附带的脚本不会自动读取该路径**；若配置此项，agent 可能按用户指令读取该文件以进行持仓市值计算。请勿将 `portfolio_file` 指向含密码、密钥等敏感信息的文件，仅放置标的代码与份额等非敏感内容。
